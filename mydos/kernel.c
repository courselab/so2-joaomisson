...
void f_help() {
  kwrite("   try also those commands:\n");
  kwrite("      exec    (execute an user program\n");
  kwrite("      list    (list files in joaomissonDOS)\n");
  kwrite("      quit    (exit joaomissonDOS)\n");
}

void f_quit() {
  kwrite("Program halted");
  go_on = 0;
}
...
void f_exec() {
  ...
  kwrite("Input the name of the program to execute: ");
